function [total_boxes points] = detect_face(img,minsize,PNet,RNet,ONet,threshold,fastresize,factor)
    ...
    save bb1.mat total_boxes
    
    sprintf('%s:%d', '[1]', size(total_boxes, 1))
    %save bb1-2.mat total_boxes
    
	numbox=size(total_boxes,1);
	if ~isempty(total_boxes)
		pick=nms(total_boxes,0.7,'Union');
		total_boxes=total_boxes(pick,:);
        sprintf('%s:%d', '[2]', size(total_boxes, 1))
        ...
        sprintf('%s:%d', '[3]', size(total_boxes, 1))
		total_boxes=rerec(total_boxes);
        sprintf('%s:%d', '[4]', size(total_boxes, 1))
		total_boxes(:,1:4)=fix(total_boxes(:,1:4));
        sprintf('%s:%d', '[4.5]', size(total_boxes, 1))
	...
	if numbox>0
        ...
        sprintf('%s:%d', '[5]', size(total_boxes, 1))
		mv=out{1}(:,pass);
        ...
        if size(total_boxes,1)>0
            ...
            pick=nms(total_boxes,0.7,'Union');
			total_boxes=total_boxes(pick,:);     
            sprintf('%s:%d', '[6]', size(total_boxes, 1))
            
            %save bb2.mat total_boxes
            
            total_boxes=bbreg(total_boxes,mv(:,pick)');	
            sprintf('%s:%d', '[7]', size(total_boxes, 1))
            total_boxes=rerec(total_boxes);
            sprintf('%s:%d', '[8]', size(total_boxes, 1))
		end
		numbox=size(total_boxes,1);
		if numbox>0
			%third stage
			...
			points=points(:,pass);
			total_boxes=[total_boxes(pass,1:4) score(pass)'];
            sprintf('%s:%d', '[9]', size(total_boxes, 1))
			mv=out{1}(:,pass);
			...
			if size(total_boxes,1)>0				
                ...
                sprintf('%s:%d', '[10]', size(total_boxes, 1))
                pick=nms(total_boxes,0.7,'Min');
				total_boxes=total_boxes(pick,:);  				
                sprintf('%s:%d', '[11]', size(total_boxes, 1))
                points=points(:,pick);
			end
		end
    end 	
